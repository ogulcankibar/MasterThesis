...
def read_time_signature(song):

    """
    songTag = taglib.File(song)
    songDict = songTag.tags
    songTimeSig = str(songDict["TIME SIGNATURE"][0])

    """

    songDict = mutagen.File(song)
    time.sleep(1)
    songTimeSig = str(songDict["TXXX:TIME SIGNATURE"][0])

    return songTimeSig

#songName = "/home/ogulcan/PycharmProjects/MasterThesis/Dataset/Veri Kümesi (Wav)/Şarkılar/Almora - Kaf Dağının Ardında/kaf_daginin_ardinda.wav"
...
for song in songs:
    timeSig = read_time_signature(song)
    songsTimeSignature.append(timeSig)