...
loader = essentia.standard.MonoLoader(filename=songs[-3])
...
analysisDelta = 1024
numFrefIter = int(math.floor((analysisEnd-analysisStart)/analysisDelta))

getWindow = essentia.streaming.Windowing(type="hann")

"""getSpectrum = essentia.streaming.Spectrum()
getFrequencyBands = essentia.streaming.FrequencyBands()
getNoveltyCurve = essentia.streaming.NoveltyCurve()
getBpmHistogram = essentia.streaming.BpmHistogram()
getBeatogram = essentia.standard.Beatogram()
getRythmExtractor = essentia.streaming.RhythmExtractor2013()
getBpmRubato = essentia.streaming.BpmRubato()"""

getBeatTrackerDegara = essentia.standard.BeatTrackerDegara()
ticksDegara = getBeatTrackerDegara(audio)
getBeatsLoudness = essentia.standard.BeatsLoudness(beats=ticksDegara)
(loudness, loudnessBandRatio) = getBeatsLoudness(audio)
beatogram = essentia.standard.Beatogram()
getMeter = essentia.standard.Meter()
meter = getMeter(beatogram(loudness, loudnessBandRatio))
